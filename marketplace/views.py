...
def home(request):
    # Main browsing page. Only approved sellers show up here.
...
def product_detail(request, pk):
    product = get_object_or_404(Product, pk=pk, is_active=True, seller__is_approved=True)
...
def checkout(request, pk):
    ...
    # This starts as a fake/test checkout.
    # Later this is where Stripe would be connected.
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if quantity < product.minimum_order_quantity:
                ...
                messages.success(request, 'Order placed. This is a test checkout, so no real payment was charged.')
...
def seller_register(request):
    # Sellers can sign up, but I still approve them manually in admin.
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            login(request, user)
            messages.success(request, 'Seller account created. An admin needs to approve it before products go live.')
            return redirect('seller_dashboard')
    else:
        form = SellerRegistrationForm()

    return render(request, 'marketplace/seller_register.html', {'form': form})
...
def add_product(request):
    ...
    if not seller.is_approved:
        messages.warning(request, 'Your seller account needs admin approval before adding products.')
        return redirect('seller_dashboard')
    ...
    if request.method == 'POST':
        ...
        form = ProductForm(initial={'city': seller.city, 'state': seller.state})
